...
public class MyUmlClass extends MyClass {
    ...
    private HashMap<String, ArrayList<UmlElement>> nameAttrAssoEndMap;
    ...
    public MyUmlClass(UmlClass umlClass) {
        ...
        this.nameAttrAssoEndMap = new HashMap<>();
    ...
    public void addAttrAssoEnd(UmlElement umlElement) {
        String name = umlElement.getName();
        if (nameAttrAssoEndMap.containsKey(name)) {
            nameAttrAssoEndMap.get(name).add(umlElement);
        } else {
            ArrayList<UmlElement> arrayList = new ArrayList<>();
            arrayList.add(umlElement);
            nameAttrAssoEndMap.put(name, arrayList);
        }
    }

    //R001：针对下面给定的模型元素容器，不能含有重名的成员(UML002)
    public Set<AttributeClassInformation> checkForUml001() {
        Set<AttributeClassInformation> set = new HashSet<>();
        for (ArrayList<UmlElement> umlElementList : nameAttrAssoEndMap.values()) {
            if (umlElementList.size() > 1) {
                if (umlElementList.get(0).getName() != null) {
                    AttributeClassInformation attributeClassInformation =
                            new AttributeClassInformation(
                                    umlElementList.get(0).getName(),
                                    umlClass.getName());
                    set.add(attributeClassInformation);
                }
            }
        }
        return set;
    ...
    public boolean checkForUml002() {
        ...
        while (myUmlClass != null) {
            if (myUmlClass.getUmlClass().getId().equals(umlClass.getId())) {
                return false;
            } else if (hashSet.contains(myUmlClass.getUmlClass().getId())) {
                return true;//非自身的环返回false，因为早晚会遍历到它
            }
            hashSet.add(myUmlClass.getUmlClass().getId());
    ...
    public boolean checkForUml004() {
        ...
        while (myUmlClass != null) {
            for (MyUmlInterface mui : myUmlClass.getImplementInterfaceList()) {
                if (implementHashSet.contains(mui)) {
                    return false;
                } else {
                    implementHashSet.add(mui);
                }
            }
            myUmlClass = myUmlClass.getExtendClass();
        }
