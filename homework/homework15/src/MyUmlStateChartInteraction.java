...
public class MyUmlStateChartInteraction implements UmlStateChartInteraction {
    ...
    //缓存
    private MyUmlAllState initialState = null;
    ...
    private void parseAllState(UmlElement umlElement) {
        ...
        if (umlElement instanceof UmlPseudostate) {
            initialState = myUmlAllState;
        }
    }
    ...
    private void parseTransition(UmlTransition umlTransition) {
        ...
        if (myUmlAllStateMap.containsKey(umlTransition.getSource()) &&
            ...
            source.addOutTransition(myUmlTransition);
        }
    ...
    private void parseEvent(UmlEvent umlEvent) {
        ...
        if (myUmlTransitionMap.containsKey(umlEvent.getParentId())) {
            ...
            myUmlTransition.addEvent(myUmlEvent);
    ...
    public void checkForUml008() throws UmlRule008Exception {
        if (initialState != null) {
            if (initialState.getMyUmlOutTransitionList().size() > 1) {
                throw new UmlRule008Exception();
            } else if (initialState.getMyUmlOutTransitionList().size() == 1) {
                MyUmlTransition myUmlTransition = initialState.getMyUmlOutTransitionList().get(0);
                if (myUmlTransition.getMyUmlEventMap().size() > 0) {
                    throw new UmlRule008Exception();
                }
                if (myUmlTransition.getUmlTransition().getGuard() != null) {
                    throw new UmlRule008Exception();
                }
            }
        }
