...
public class MyUmlStateMachine {
    ...
    输出：
    Ok，triggers of transition from state "open" to state "close" in statemachine
     "door_sm" are (A, B, C). A、B、C为应状态机模型door_sm中引起状态open迁移到状态close的触发事件，其中
    ...
    Failed, transition from state "open" to state "close" in statemachine
    "door_sm" not found.在状态机模型door_sm中未找到从状态open到状态close的迁移
    说明：
    该询问考虑的迁移为状态间的直接迁移。
