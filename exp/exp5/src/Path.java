...
public class Path implements Iterable<Integer> {        // 包含题目[10]-[14]
    ...
    /*@ public normal_behavior
      ...
      @ assignable \nothing;
      @ signals (IndexOutOfBoundsException e) index < 0 || index >= size(); //TODO[10]
      @*/
    ...
    //@ ensures \result == (\exists int i; 0 <= i < nodes.length; nodes[i] == nodeId);    //TODO[11]  请使用一条\forall或\exists实现，量词只允许出现一次
    ...
    public /*@pure@*/ boolean equals(final Object obj) {
        ...
        while (iterator.hasNext() || myIterator.hasNext()) {
            if (!iterator.next().equals(myIterator.next())) {  //TODO[12]
                return false;
            }
        }
        return true;
    ...
    public /*@pure@*/ boolean isValid() {
        return (size() >= 2 && size() - distinct.size() <= 1);   //TODO[13]  请使用类中的属性(及其方法)和运算符实现
    }
    ...
    public /*@pure@*/ Path extractLoopPath() {
        ...
        loopStartIndex = nodeToIndex.get(nodes.get(index)); //TODO[14]
        loopLastIndex = index;
