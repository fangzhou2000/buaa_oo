...
public class PathContainer {        // 包含题目[2]-[9]
    ...
    //@ ensures \result == pList.length; //TODO[2]
    ...
    /*@ ensures \result == (\exists int i; 0 <= i && i < pidList.length; //TODO[3]
      @                      pidList[i] == pathId);
      @*/
    ...
    /*@ public normal_behavior
      ...
      @ also
      @ public exceptional_behavior //TODO[4]
      @ requires !containsPathId(pathId);
      @ assignable \nothing;
    ...
    /*@ public normal_behavior
      ...
      @ requires path == null || path.isValid() == false || !containsPath(path); //TODO[5]
    ...
    public /*@pure@*/ int getPathId(Path path) throws PathNotFoundException {
        if (path != null && path.isValid() && containsPath(path)) {
            ...
            throw new PathNotFoundException(path); //TODO[6]
        }
    }
    ...
    public int addPath(Path path) {
        if (path != null && path.isValid() && !containsPath(path)) {
            ...
            for (int node : path) {
                Integer prev = globalNodesCount.get(node);  //TODO[7]不确定
                if (prev == null) {
    ...
    public void removePathById(int pathId) throws PathIdNotFoundException {
        if (containsPathId(pathId)) {
            ...
            for (int node : path) {
                ...
                globalNodesCount.remove(node, prev);  //TODO[8]
            }
        } else {
            throw new PathIdNotFoundException(pathId);
        }
